...
#剪枝过程分为三步：进行稀疏化诱导训练，进行剪枝得到剪枝后的模型，用剪枝之后的模型作为预训练模型在正常训练一次
if __name__ == "__main__":
    ...
    #获取参与剪枝层次的索引，具体实现参考utils/prune_utils.py
    #得到不是shortcut层输入的CBL层的索引，因为shortcut层实现元素相加，如果参与剪枝，其输入层剪枝结果不一致会导致shortcut层不能实现
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_i, (_, imgs, targets) in enumerate(dataloader):
            ...
            #model的实现参考model.py，注意的是这里返回的loss是正常训练的loss，还不包括稀疏性正则项
            loss, outputs = model(imgs, targets)

            optimizer.zero_grad()
            loss.backward()
            #更新BN层的scales具体实现参考utils/prune_utils.py
