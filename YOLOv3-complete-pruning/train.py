...
def train():
    ...
    #将前面得到的pg1添加到优化器组中，指定对应的权重衰减参数
    #也就是给所有的卷积层权重应用权重衰减，防止过拟合
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        #加载darknet的yolov3权重，返回backbone截止的层次id
    ...
    #设置调整学习率的方式，分别在epochs*0.8和epochs*0.9的时候将学习率调整成当前的0.1倍
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[round(opt.epochs * x) for x in [0.8, 0.9]], gamma=0.1)
    #如果加载的是darknet的权重start_epoch是0
    ...
    if hasattr(model, 'module'):
        ...
        if opt.prune==1:
            print('shortcut sparse training')
            #对shortcut层进行剪枝，返回包括shortcut层前面的卷积模块在内的剪枝层次
            _,_,prune_idx,_,_=parse_module_defs2(model.module_defs)
        elif opt.prune==0:
            print('normal sparse training')
            #正常的剪枝和规整剪枝,具体实现在utils/prun_utils.py
            #返回参与剪枝的层次的索引，主要是包括BN的卷积层的索引，其中shortcut和upsample层前面的卷积层，yolo层前面的卷积层不参与剪枝
            #也就是利用BN层参数进行剪枝的那一版
            _,_,prune_idx= parse_module_defs(model.module_defs)
        elif opt.prune==2:
            #对tiny模型剪枝,返回不包括upsample前免得CBL模块的id
            print('tiny yolo normal sparse traing')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #梯度调整
            if hasattr(model, 'module'):
                #参考utils/prune_utils.py中的实现
                #这里调整BN层中的scales的梯度，添加稀疏化惩罚的梯度
